...
from associated lending markets.
...
The table above lists the average APY for Curve Finance's 3pool within 137 days
(data covers the range of 2022-09-21 to 2023-02-04, data from DefiLlama)
and for the Dynamo base pool for the same time range.

As can be seen, the Dynamo APY value for the base pool is higher compared to Curve's 3pool.
Hence, we outperform a well-established liquidity pool by shifting idle pool liquidity
to lending markets to activate the full APY potential even without including liquidity incentives.
Once we include liquidity mining rewards (approx. 1.5-3.6%, values taken from the bb-a-USD pool),
we are able to obtain a competitive APY value within the current DeFi market for a liquidity
pool.
